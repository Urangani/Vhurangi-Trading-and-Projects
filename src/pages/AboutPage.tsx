    ...
    <div>
      ...
      <div className="bg-neutral-800 text-white py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 text-center relative z-10">
          ...
          <div className="w-20 h-1 bg-amber-500 mx-auto mt-4 animate-scaleIn"></div>
        </div>
      </div>
      ...
      <section ref={storyRef} className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
          <div className="story-content">
            ...
            <div className="w-16 h-1 bg-amber-500 mt-3 mb-6"></div>
            ...
            <div className="mt-6 flex gap-4">
              <div className="flex items-center gap-2"><Star className="text-amber-500" size={20} /><span>500+ Projects Completed</span></div>
              <div className="flex items-center gap-2"><Heart className="text-amber-500" size={20} /><span>98% Client Satisfaction</span></div>
            </div>
          </div>
...
const ValueCard = ({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) => {
  ...
  return (
    <div ref={cardRef} className="value-card bg-white rounded-xl p-8 text-center shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
      <div className="text-amber-600 w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-amber-50 rounded-full">{icon}</div>
...
const TeamCard = ({ name, role, image }: { name: string; role: string; image: string }) => {
  ...
  return (
    ...
    >
      <div className="w-48 h-48 bg-neutral-300 rounded-full mx-auto mb-5 bg-cover bg-center border-4 border-amber-500 shadow-lg transform transition-transform duration-300 hover:scale-110" style={{ backgroundImage: `url(${image})` }}></div>
      <h3 className="font-bold text-xl">{name}</h3>
      <p className="text-amber-600 font-medium">{role}</p>
    </div>
  );
};
